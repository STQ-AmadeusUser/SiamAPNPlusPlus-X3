...
class ADSiamAPNTracker(object):
    ...
    def generate_grids(self):
        size = 21
        self.grid_x = np.tile(
            (cfg.ANCHOR.STRIDE * (np.linspace(0, size-1, size))) - cfg.ANCHOR.STRIDE * (size-1) / 2, size
        ).reshape(-1)
        self.grid_y = np.tile(
            (cfg.ANCHOR.STRIDE * (np.linspace(0, size-1, size))).reshape(-1, 1) - cfg.ANCHOR.STRIDE * (size-1) / 2, size
        ).reshape(-1)
    ...
    def track(self, img, gt=None, counter=None):
        """
        args:
            img(np.ndarray): BGR image
        return:
        ...
        mapp = outputs['anchor']  # 1x4x21x21
        size = 21
        shap = self.con(mapp.squeeze())  # 4x21x21
        ...
        score1 = outputs['cls1'].squeeze() * cfg.TRACK.w1
        score2 = outputs['cls2'].squeeze() * cfg.TRACK.w2
        score3 = outputs['cls3'].squeeze() * cfg.TRACK.w3
        score = (score1 + score2 + score3) / 3

        delta = outputs['loc'].squeeze().reshape(4, size ** 2)
