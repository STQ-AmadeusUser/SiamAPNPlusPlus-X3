...
from pysot.tracker.adsiamapn_tracker_X3 import ADSiamAPNTracker
...
def parse_args():
    ...
    parser.add_argument('--inference', default='../bin/SiamAPN++.bin', help='SiamAPN++ algorithm in Aerial Tracking algorithms')
...
def main():
    ...
    print_properties(inference[0].outputs[3].properties)
    print_properties(inference[0].outputs[4].properties)

    print('===> create inference model <====')
    net = ModelBuilder(model=dnn_model)

    print('===> build tracker <====')
    tracker = ADSiamAPNTracker(net)
